...
const ACCEPT_IMAGE_TYPE = ['image/png', 'image/jpeg', 'image/gif'];
const ACCEPT_IMAGE_SIZE = 2 * 1024 * 1024; // 2MB
...
function setSelectInput(selector, defaultIndex) {
  let selectedType = $(selector).attr('value');
  const types = $.map($(selector + ' option'), function (el) {
    return el.value;
  });
...
function pagination(totalCount, offset, handler, isVueComponent = false) {
  ...
  const pageLine = PAGE_LINE;
  const maxPageButton = MAX_PAGE_BUTTON;
  const clickEvent = isVueComponent ? '@click.native' : 'onclick';
  let html = '';
  ...
  ${totalCount == 0 ? '検索結果はありません。' :
    ...
    for (let i = 0; i < totalCount; i += pageLine) {
      ...
      if (breakPaging && ignoreCondition) {
        const temp = `<li class="paginate_button disabled"><a href="#" onclick="" >...</a></li>`;
...
function showPopupHandler(event) {
  ...
  const btnReset = $(target).find('button.btn-reset').eq(0);
  const btnSearch = $(target).find('button.btn-search').eq(0);
  $(btnReset).click();
  // reset data table
  setTimeout(function () {
    $(btnSearch).click();
...
$(document).ready(function () {
  ...
  $('.pageLimitSetting select').change(function (event) {
    const href = $(this).data('href') + '&pageLine=' + $(this).val();
...
function hideDataTable(modal) {
  ...
  const table = modal.find('.table-container:eq(0) table tbody, .paginate-container:eq(0)>.col>.btn-group');
  table.hide();
}
...
function eventBack(returnUrl) {
  ...
  $("form input:visible, form select:visible").each(function (input) {
    ...
    let hdn = $("input[name='" + "hdn" + jsUcfirst(name) + "']:hidden, select[name='" + "hdn" + jsUcfirst(name) + "']:hidden").val();
...
jQuery.fn.extend({
  common: {
    ...
    timer: function (downloadToken) {
      let attempts = 30;    // wait for maximum 20 seconds
      const downloadTimer = window.setInterval(function () {
        const token = $.fn.common.getCookie("downloadToken");
        attempts--;
    ...
    getCookie: function (name) {
      ...
      const parts = parse(document.cookie);
