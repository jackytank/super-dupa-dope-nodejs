...
class AdminUserController {
    ...
    async createNewUser(req: Request, res: Response) {
        ...
        try {
            const result: CustomApiResult<User> = await this.userRepo.insertData(
                user,
                null,
                queryRunner,
                { wantValidate: true, isPasswordHash: true },
            );
            if (result.status === 400 || result.status === 500) {
                await queryRunner.rollbackTransaction();
                req.flash(
                    'message',
                    result.message ?? 'Error when create user!',
                );
    ...
    async editPage(req: Request, res: Response) {
        const { id } = req.params;
        const result: CustomApiResult<User> = await this.userRepo.getOneData(
            parseInt(id),
        );
    ...
    async update(req: Request, res: Response) {
        ...
        if (role !== ROLE.USER + '') {
            if (
                req.session.authority !== ROLE.ADMIN &&
                req.session.authority !== ROLE.MANAGER
            ) {
                req.flash(
                    'message',
        ...
        try {
            const result: CustomApiResult<User> = await this.userRepo.updateData(
                user,
                null,
                queryRunner,
                { wantValidate: true },
    ...
    async changePassword(req: Request, res: Response) {
        ...
        try {
            ...
            req.flash(
                'message',
                error.message ??
                    'Error when trying to update password! Please try again',
            );
            res.redirect(`/admin/users/edit/${id}`);
        } finally {
            await queryRunner.release();
