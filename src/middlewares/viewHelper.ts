...
export default async (req: Request, res: Response, next: NextFunction) => {
    ...
    const newPagination = (totalCount: number) => {
        ...
        if (totalCount > 0) {
            const breakPaging =
                Math.floor(totalCount / pageLine) > maxPageButton + 2;
            const breakCount = [];
            if (breakPaging) {
                breakCount.push(Math.floor(maxPageButton / PAGING_DIVISER));
                breakCount.push(
                    Math.floor((maxPageButton - 1) / PAGING_DIVISER),
                );
            }
            ...
            for (let i = 0; i < totalCount; i += pageLine) {
                ...
                if (breakPaging && ignoreCondition) {
                    ...
                    if (
                        html.slice(html.length - temp.length, html.length) !==
                        temp
                    ) {
                        // 追加したかどうか確認する
                        html += temp;
            ...
            if (offset + pageLine < totalCount) {
                query.offset =
                    Math.floor((totalCount - 1) / pageLine) * pageLine;
                href = `${req.baseUrl}?${qs.stringify(query)}`;
            }
            html += `</ul></div>`;
        } else {
